...
def getPaths():
    ...
    # where we load numpy (npy) data from about the halfscreen crosses and the stored indices data
    dataLoadPath = os.path.join(curDir, '..', '..', '..', 'results', 'halfscreenCrosses', 'halfscreen.npy')
    indicesLoadPath = os.path.join(curDir, '..', '..', '..', 'results', 'halfscreenCrosses', 'indices.csv')
...
def main():
    ...
    print('            Runtime for extracting halfscreen crosses Pearson CSV data: %.4f seconds'%(time.time() - startTime))
