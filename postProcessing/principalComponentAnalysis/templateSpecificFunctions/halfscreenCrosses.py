...
def getPaths():
    ...
    loadPath = os.path.join(curDir, '..', '..', '..', 'results', 'halfscreenCrosses', 'results')
...
def main():
    ...
    for templateName in os.listdir(loadPath):
        for distanceScheme in distanceSchemes:
            ...
            data = collectData(loadPath, 'halfscreen', templateName, distanceScheme)
    ...
    print('            Runtime for halfscreen Principal Component Analysis: %.4f seconds'%(time.time() - startTime))
